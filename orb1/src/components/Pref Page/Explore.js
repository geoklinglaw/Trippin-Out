...
function Explore () {
    ...
    return (
        ...
        <div style={{
                ...
                <h1 style={{marginLeft: '17px', fontSize: '40px', color: '#1C395B'}}>Explore</h1>
            </div>
            ...
            <div style={{ marginLeft: 'auto', marginRight: '45px', marginTop: '90px' }}>
                <Search
                placeholder="Search for an itinerary"
                onSearch={handleSubmit} onChange={handleSearch}
                style={{
                    width: 250,
                }}
                />
        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '20px' }}>
                {itinerary.map((itin) => (
                    <SuggItin key={itin.id} pic = {itin.link} title={itin.title} description={itin.description} />
                ))}
                </div>
              </div>
            </div>
